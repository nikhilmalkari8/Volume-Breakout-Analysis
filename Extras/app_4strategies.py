...
def create_plotly_plot(data, trade_days, strategy_name, results):
    ...
    fig.add_trace(go.Scatter(x=trade_days.index, y=trade_days['Close'], mode='markers', name='Buy Point', marker=dict(color='green', symbol='triangle-up', size=10)))
    sell_dates = pd.to_datetime(results['Sell Date'].dropna())
    sell_prices = results['Sell Price'].dropna()
    fig.add_trace(go.Scatter(x=sell_dates, y=sell_prices, mode='markers', name='Sell Point', marker=dict(color='red', symbol='triangle-down', size=10)))
